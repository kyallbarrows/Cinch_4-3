        ...
        public void SolveVelocityConstraints()
        {
            for (int i = 0; i < _constraintCount; ++i)
            {
                ...
                for (int j = 0; j < c.PointCount; ++j)
                {
                    ...
                    float newImpulse = Mathf.Max(-maxFriction, Mathf.Min(ccp.TangentImpulse + lambda, maxFriction));
        ...
        public bool SolvePositionConstraints(float baumgarte)
        {
            ...
            for (int i = 0; i < _constraintCount; ++i)
            {
                ...
                for (int j = 0; j < c.PointCount; ++j)
                {
                    ...
                    float C = Mathf.Max(-Settings.MaxLinearCorrection,
                                       Mathf.Min(baumgarte * (separation + Settings.LinearSlop), 0.0f));
                ...
                case ManifoldType.Circles:
                    {
                        ...
                        if (a > Settings.Epsilon * Settings.Epsilon)
                        {
                            ...
                            float factor = 1f / Mathf.Sqrt(normalTmp.x * normalTmp.x + normalTmp.y * normalTmp.y);
